...
import rgvn_utils
import utils
from nlp_processor import BagOfWords
from nlp_processor import TextPreProcessor
from nlp_processor import Word2Vec
from nlp_processor import TfidfEmbeddingVectorizer
from tensorflow.contrib import learn
from sgcn_utils import generate_id_for_sgcn,SGCNData

def load_tabular_data(data_name, dict_no, noise_rate, vocab_size,feature_method,is_merge_col):
    ...
    # Blog Feedback dataset
        ...
        # Load train dataset
        data_train = pd.read_csv("../text_gcn/data/corpus/cut_20ng/"+data_name+"_train.txt",'\t', header=None, quoting=csv.QUOTE_NONE, encoding='utf-8')
        data_train.columns = ['label', 'text_all']
        # Loads test dataset
        data_test = pd.read_csv("../text_gcn/data/corpus/cut_20ng/"+data_name+"_test.txt",'\t', header=None, quoting=csv.QUOTE_NONE, encoding='utf-8')
        data_test.columns = ['label', 'text_all']
        ...
        df['Y'] = df['Y'].astype(int)
        print("Load 20ng dataset successfully!")

        if feature_method == "word2vec":
            print("Use avg vector of word2vec from googlenews as classification features!")
            word_vecs = gensim.models.KeyedVectors.load(
                '../data/GoogleNews_vectors-negative300_uncompress_noL2Norm', mmap='r')
            word2vec = Word2Vec(word_vecs, 300)
            df_test_all_W2V = pd.DataFrame((word2vec.transform(df['text_all'].tolist())))
            df_title_W2V = pd.DataFrame((word2vec.transform(df['title'].tolist())))
            df_description_W2V = pd.DataFrame((word2vec.transform(df['description'].tolist())))
        elif feature_method == "tfIdf_word2vec":
            print("Use tfIDF word2vec as classification features!")
            word_vecs = gensim.models.KeyedVectors.load(
                '../data/GoogleNews_vectors-negative300_uncompress_noL2Norm', mmap='r')
            word2vec = TfidfEmbeddingVectorizer(word_vecs,300)
            df_test_all_W2V = pd.DataFrame((word2vec.fit_transform(df['text_all'].tolist())))
            df_title_W2V = pd.DataFrame((word2vec.fit_transform(df['title'].tolist())))
            df_description_W2V = pd.DataFrame((word2vec.fit_transform(df['description'].tolist())))
        elif feature_method == "BOW":
            print("Use Bag of Words as classification features!")
            df_test_all_W2V = BagOfWords.generateBOW(df['text_all'].tolist(),vocab_size)
            df_test_all_W2V = pd.DataFrame(np.matrix(df_test_all_W2V))
            df_title_W2V = BagOfWords.generateBOW(df['title'].tolist(),vocab_size)
            df_title_W2V = pd.DataFrame(np.matrix(df_title_W2V))
            df_description_W2V = BagOfWords.generateBOW(df['description'].tolist(),vocab_size)
            df_description_W2V = pd.DataFrame(np.matrix(df_description_W2V))
        elif feature_method == "sgcn":
            LogUtil.log("INFO", "Use sgcn as classification features.")
            vocab_dic_path = "../text_gcn/data/corpus/"+data_name+"_vocab.txt"
            vocab_dic = dict()
            window = 4
            f = open(vocab_dic_path, 'r')
            lines = f.readlines()
            idx = 0
            for line in lines:
            # Preseve "0" for "unknown" word (OOV)
                idx +=1
                temp = line.strip()
                vocab_dic[temp] = idx
            vocab_count = idx
            x_train = df['text_all'].tolist()
            y_train = df['Y'].tolist()
            x_train_sgcn_ids = generate_id_for_sgcn(x_train, vocab_dic, dataset=data_name)
            LogUtil.log("INFO","Generated batch ids for sgcn.")
            LogUtil.log("INFO","Finished sgcn inference.")
            x_train_sgcn_ids = np.squeeze(x_train_sgcn_ids)
            df_test_all_W2V = pd.DataFrame(x_train_sgcn_ids)

        LogUtil.log("Info","Start to merge feature with original text data!")
        if is_merge_col:
            LogUtil.log("Info", "20ng, Merge Title and Description into one unit feature!")
            df = pd.merge(df, df_test_all_W2V, left_index=True, right_index=True)
        else:
            LogUtil.log("Info", "20ng, Seperate Title and Description to 2 units feature!")
            df = pd.merge(df, df_title_W2V, left_index=True, right_index=True)
            df = pd.merge(df, df_description_W2V, left_index=True, right_index=True)
        LogUtil.log("Info","End to merge feature with original text data!")
        # df = df.drop(['title','description'],axis=1)

    elif data_name == "ag_news":
        # Load train dataset
        data_train = pd.read_csv('../data/ag_news_csv/train.csv','","',header=None, quoting=csv.QUOTE_NONE, encoding='utf-8')
        data_train.columns = ['label', 'title', 'description']
        # Loads test dataset
        data_test = pd.read_csv("../data/ag_news_csv/test.csv",'","', header=None, quoting=csv.QUOTE_NONE, encoding='utf-8')
        data_test.columns = ['label', 'title', 'description']
        ...
        print("Start to merge title and description to preprocess data!")
        df['text_all'] = df['title'] +" "+df['description']
        # df.text_all = df.text_all.map(lambda x: TextPreProcessor.clean_str(x))
        if feature_method == "word2vec":
            print("Use avg vector of word2vec from googlenews as classification features!")
            word_vecs = gensim.models.KeyedVectors.load(
                '../data/GoogleNews_vectors-negative300_uncompress_noL2Norm', mmap='r')
            ...
            LogUtil.log("INFO", "Use sgcn as classification features.")
            vocab_dic_path = "../DocumentGCN/tensorflow/ag_news_vocab.txt"
            model_path = "../DocumentGCN/tensorflow/sgcn_runs/1588852267/checkpoints/"
            vocab_dic = dict()
            window = 4
            f = open(vocab_dic_path, 'r')
            lines = f.readlines()
            for line in lines:
                ...
                vocab_dic[temp[0]] = int(temp[1])
            x_train = df['text_all'].tolist()
            y_train = df['Y'].tolist()
            x_train_sgcn_ids = generate_id_for_sgcn(x_train, vocab_dic, dataset="ag_news")
            LogUtil.log("INFO","Generated batch ids for sgcn.")
            LogUtil.log("INFO","Finished sgcn inference.")
            x_train_sgcn_ids = np.squeeze(x_train_sgcn_ids)
            df_test_all_W2V = pd.DataFrame(x_train_sgcn_ids)
        ...
        LogUtil.log("Info","Start to merge feature with original text data!")
        if is_merge_col:
            LogUtil.log("Info", "Ag News, Merge Title and Description into one unit feature!")
            df = pd.merge(df, df_test_all_W2V, left_index=True, right_index=True)
        else:
            LogUtil.log("Info", "Ag News, Seperate Title and Description to 2 units feature!")
            df = pd.merge(df, df_title_W2V, left_index=True, right_index=True)
            df = pd.merge(df, df_description_W2V, left_index=True, right_index=True)
        LogUtil.log("Info","End to merge feature with original text data!")
        df = df.drop(['title','description'],axis=1)
    ...
    y_train, noise_idx = rgvn_utils.corrupt_label(y_train, noise_rate)
    train['Y'] = y_train

    # Saves data
    timestamp = str(int(time.time()))
    base_file_path = os.path.abspath(os.path.join(os.path.curdir, "data_files", timestamp))
    if not os.path.exists(base_file_path):
        os.makedirs(base_file_path)
    train_file_path = os.path.abspath(os.path.join(base_file_path,'train.csv'))
    valid_file_path= os.path.abspath(os.path.join(base_file_path,'valid.csv'))
    test_file_path = os.path.abspath(os.path.join(base_file_path,'test.csv'))
    ...
    return noise_idx,vocab_count, train_file_path,valid_file_path,test_file_path
...
def load_rossmann_data(dict_no, setting, test_store_type):
    ...
    # Defines store types
    data_c = data_x[['StoreType_a', 'StoreType_b',
...
def process_data_for_sgcn(train_file_name, valid_file_name, test_file_name,vocab_dic_path,model_path,dataset="ag_news"):
    LogUtil.log("INFO","Start to inference with sgcn.")
    train = pd.read_csv(train_file_name)
    valid = pd.read_csv(valid_file_name)
    ...
    for line in lines:
        temp = line.strip().split("\t")
        vocab_dic[temp[0]] = temp[1]

    LogUtil.log("INFO","Training data start to inference with sgcn.")
    x_train = train['text_all'].tolist()
    x_train_sgcn_ids = generate_id_for_sgcn(x_train,vocab_dic,dataset)
    x_train_embedding = eval_sgcn(x_train_sgcn_ids,window,model_path)

    LogUtil.log("INFO","Valid data start to inference with sgcn.")
    x_valid = valid['text_all'].tolist()
    x_valid_sgcn_ids = generate_id_for_sgcn(x_valid,vocab_dic,dataset)
    x_valid_embedding = generate_id_for_sgcn(x_valid_sgcn_ids,window,model_path)

    LogUtil.log("INFO","Test data start to inference with sgcn.")
    x_test = test['text_all'].tolist()
    x_test_sgcn_ids = generate_id_for_sgcn(x_test,vocab_dic,dataset)
    x_test_embedding = generate_id_for_sgcn(x_test_sgcn_ids, window, model_path)

    y_train = np.asarray(train['Y'])
    y_valid = np.asarray(valid['Y'])
    y_test = np.asarray(test['Y'])

    return x_train_embedding,y_train,x_valid_embedding,y_valid,x_test_embedding,y_test

def process_data_for_textCNN(train_file_name, valid_file_name, test_file_name):
    train = pd.read_csv(train_file_name)
    valid = pd.read_csv(valid_file_name)
    test = pd.read_csv(test_file_name)
    train.text_all = train.text_all.map(lambda x: TextPreProcessor.clean_str(x))
    valid.text_all = valid.text_all.map(lambda x: TextPreProcessor.clean_str(x))
    test.text_all = test.text_all.map(lambda x: TextPreProcessor.clean_str(x))
    x_train = train['text_all'].tolist()
    ...
    x_valid = np.array(list(vocab_processor.transform(valid['text_all'])))
    y_valid = np.array(valid['Y'])
    y_valid = np.eye(len(np.unique(y_valid)))[y_valid.astype(int)]

    x_test = np.array(list(vocab_processor.transform(test['text_all'])))
    y_test = np.array(test['Y'])
    y_test.astype(int)

    return x_train,y_train,x_valid,y_valid,x_test,y_test,vocab_processor


def preprocess_data(dataset, normalization,
    ...
    if normalization == 'minmax':
        ...
        LogUtil.log("INFO","Normalizing data with a standard manner")
    ...
    if dataset == 'ag_news' or dataset == '20ng':
        import ast
        df['0'] = df['0'].apply(lambda x:ast.literal_eval(x))
        if not type(df) == np.ndarray:
            df = df.to_numpy()
        x_train = df[range(train_no), 0]
        x_valid = df[range(train_no, train_no + valid_no), 0]
        x_test = df[range(train_no+valid_no, train_no+valid_no+test_no), 0]
    else:
        if not type(df) == np.ndarray:
            df = df.to_numpy()
...
if __name__ == '__main__':
    preprocess_data('ag_news',None,'../data_files/1593081060/train.csv',\
                    '../data_files/1593081060/valid.csv',\
                    '../data_files/1593081060/test.csv')
